...
paths = Counter()
statuses = Counter()
ip_paths = defaultdict(lambda: deque(maxlen=10))
total = 0
...
print("\n🌍 All IPs (with location and last 10 routes):")
for ip, count in ips.most_common():
    location = get_geo(ip)
    recent = list(ip_paths[ip])
    print(f"\n  {ip:15} → {count:4} requests from {location}")
    for route in recent:
        print(f"      - {route}")
